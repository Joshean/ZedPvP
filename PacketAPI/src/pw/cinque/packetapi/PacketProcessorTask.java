...
class PacketProcessorTask extends BukkitRunnable {
    ...
    public PacketProcessorTask(PacketAPI plugin) {
		this.plugin = plugin;
	}

	public List<PacketPlayer> getPlayers() {
		return players;
	}

	public void setPlayers(List<PacketPlayer> players) {
		this.players = players;
	}

	public PacketAPI getPlugin() {
		return plugin;
	}
    ...
    private void processQueues(PacketPlayer packetPlayer) {
        packetPlayer.getIncomingQueue().removeIf(packet -> {
            ...
            if (wrappedPacket != null) {
                PacketReceiveEvent event = new PacketReceiveEvent(wrappedPacket, packetPlayer.getPlayer());
        ...
        packetPlayer.getOutgoingQueue().removeIf(packet -> {
            ...
            if (wrappedPacket != null) {
                PacketSendEvent event = new PacketSendEvent(wrappedPacket, packetPlayer.getPlayer());
                Bukkit.getPluginManager().callEvent(event);
                return event.isCancelled();
            } else {
                return false;
